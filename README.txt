...
-- ADDITIONAL NOTES -- 
**IMPORTANT** Depending on your screen size, larger arenas may have some of their tiles partly offscreen, due to how drawapp blocks out the bottom part of the window with the output window. If you are running this on a smaller display and notice that some tiles do not fully appear in the window, change the max, mid, min tilesize constants in the constants file. By default these are set to 15,10,5 (to accommodate the laptop I used). On the other hand, with a larger monitor it is worth increasing tilesize to make the animation look better and more visible. On my 1920x1080 24 inch monitor, 20,10,5 worked excellently. Make sure to save and recompile! This should not be an issue on normal-sized monitors.
...
If you would like the robot to move faster, lower the "cooldown" constant in the constants file. If you would like to generate more markers when doing randomgen,
you can change the MARKER_COUNT constant as well, however do not make it too high or there will be an error.
